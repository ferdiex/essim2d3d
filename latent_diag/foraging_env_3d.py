...
class Foraging3DEnv:
    def __init__(self, cfg=None, world_name="random_obstacles", num_threads=1,
                 ...
                 swap_creation_order=False, stall_activation_threshold=25):
        ...
        # stall_activation_threshold (sesion Paper 3, espacio latente):
        # parametrizado para permitir explorar valores distintos a 25 sin
        # tocar el comportamiento del Paper 2 (que usa el default=25, el
        # mismo valor heredado sin recalibracion explicita -- ver nota en
        # Paper 2, seccion de limitaciones/trabajo futuro). Pasar un valor
        # explicito aqui para las corridas de exploracion del Paper 3;
        # dejarlo en el default reproduce exactamente el comportamiento
        # usado para los resultados ya publicados del Paper 2.
        self.stall_activation_threshold = stall_activation_threshold
    ...
    def step(self, controllers, signals, mute_social=None, h_prob=0.0, reverse_order=False):
        ...
        for i, rid in enumerate(self.robots):
            ...
            if self.prev_positions[i] is not None:
                ...
                if self.stall_counts[i] >= self.stall_activation_threshold:
                    self.stalled[i] = True
